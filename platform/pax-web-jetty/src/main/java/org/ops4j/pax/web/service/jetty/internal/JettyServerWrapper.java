 ...
 *
 * <p>Copyright 2007 Alin Dreghiciu.
 *
...
/**
 * This file was taken from org.ops4j.pax.web.service.jetty.internal.JettyServerWrapper. Our intent
 * is to make as little change as possible to this file. Search for "Replacement code" for comment
 * where necessary change was made. Link to form post addressing issue:
 * https://groups.google.com/forum/#!topic/ops4j/4XkeOKL7L6k
 */
...
class JettyServerWrapper extends Server {
  ...
  JettyServerWrapper(ServerModel serverModel, ThreadPool threadPool) {
    ...
    if (jettyBundle != null) {
      ...
      packageAdminTracker =
          new ServiceTracker<>(jettyBundle.getBundleContext(), filterPackage, null);
  ...
  private HttpServiceContext addContext(final ContextModel model) {
    ...
    if (packageAdminTracker != null) {
      ServletContainerInitializerScanner scanner =
          new ServletContainerInitializerScanner(
              bundle, jettyBundle, packageAdminTracker.getService());
    ...
    // These are the lines that we want to replace to not specify AuthMethod in every WEB.xml:
    ...
    Authenticator auth = getAuthenticator("DDF"); // new JettyAuthenticator(); // here
    final SecurityHandler securityHandler = context.getSecurityHandler();
    if (auth != null) securityHandler.setAuthenticator(auth);
    if (model.getRealmName() != null) securityHandler.setRealmName(model.getRealmName());
  ...
  private Authenticator getAuthenticator(String method) {
    ServiceLoader<AuthenticatorService> sl =
        ServiceLoader.load(AuthenticatorService.class, getClass().getClassLoader());
  ...
  /**
   * Returns a list of servlet context attributes out of configured properties and attribues
   * containing the bundle context associated with the bundle that created the model (web element).
   *
  ...
  /**
   * Configures the session time out by extracting the session handlers->sessionManager for the
   * context.
   *
   ...
   * @param path Session cookie path. default to the current servlet context path.
   * @param url session URL parameter name. Defaults to jsessionid. If set to null or "none" no URL
   *     rewriting will be done.
   * @param cookieHttpOnly configures if the Cookie is valid for http only and therefore not
   *     available to javascript.
   * @param secure Configures if the session cookie is only transfered via https even if its created
   *     during a non-secure request. Defaults to false which means the session cookie is set to be
   *     secure if its created during a https request.
   * @param workerName name appended to session id, used to assist session affinity in a load
   *     balancer
