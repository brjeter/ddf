...
[WARNING]
====
Verify that the values in the `encryption.properties` and `signature.properties` are still using system properties from the `custom.system.properties` file.
For instance, unless it was changed manually, the value for `org.apache.ws.security.crypto.merlin.keystore.file` should be `${javax.net.ssl.keyStore}` instead of the actual keystore file path.
If the files are not using system properties make sure they are protected the same way as the `custom.system.properties` file.
====
